...
const DEFAULT_CHARACTERS = [
  ...
  },
  {
    ...
    systemPrompt: `You are Ryoji Mochizuki from Persona 3. You are charming, philosophical, and melancholic. You speak poetically, often referencing impermanence. You are Death itself given human form, and find humanity fascinating. Keep responses concise (2-4 sentences).`,
  ...
  },
  {
    ...
    systemPrompt: `You are Mitsuru Kirijo from Persona 3. You are elegant, authoritative, and carry the weight of your family's sins. You speak formally with precision but are deeply caring beneath the surface. Keep responses concise (2-4 sentences).`,
  ...
  },
  {
    ...
    greeting: "Yeah, I wrote TempleOS. Took me ten years. It's a 64-bit operating system — the Third Temple, for God. You want to talk programming or theology? I do both.",
    isDefault: true,
  },
];
...
const COLOR_PRESETS = [
  "#4a8fc0","#e8c84a","#c0392b","#6b4fa0","#00cc44","#e67e22",
  "#1abc9c","#e91e63","#ff5722","#607d8b","#795548","#9c27b0",
];
...
const saveCharacters = (chars) => {
  try { localStorage.setItem("persona_characters_v2", JSON.stringify(chars)); } catch {}
};
...
export default function PersonaChat() {
  ...
  useEffect(() => { setTimeout(() => setShowMoon(true), 300); }, []);
  ...
  const openCreate = () => {
    ...
    setSearchQuery(""); setSearchStatus(""); setEditingChar(null); setPhase("create");
  };

  const openEdit = (char, e) => {
    e.stopPropagation();
    setForm({ ...char });
    setSearchQuery(char.name); setSearchStatus(""); setEditingChar(char); setPhase("edit");
  };
  ...
  const deleteChar = (id) => { setCharacters(prev => prev.filter(c => c.id !== id)); setDeleteConfirm(null); };

  const searchAndBuild = async () => {
    if (!searchQuery.trim()) return;
    setIsSearching(true); setSearchStatus("🔍 Searching the web...");
    try {
  ...
  const selectCharacter = (char) => {
    setSelectedChar(char); setMessages([{role:"assistant",content:char.greeting,id:Date.now()}]);
    setPhase("chat"); setTimeout(() => inputRef.current?.focus(), 300);
  };

  const sendMessage = async () => {
    if (!input.trim()||isTyping) return;
    const userMsg = {role:"user",content:input.trim(),id:Date.now()};
    setInput(""); setMessages(prev=>[...prev,userMsg]); setIsTyping(true);
    try {
      const history = [...messages,userMsg].map(m=>({role:m.role,content:m.content}));
      const res = await fetch("/api/chat",{
        method:"POST", headers:{"Content-Type":"application/json"},
        body:JSON.stringify({system:selectedChar.systemPrompt,messages:history}),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Request failed");
    ...
    <>
      ...
      <div className="p3app" style={{
        ...
        <div className="p3moon-wrap" style={{opacity:showMoon?1:0}}>
          ...
          <div className="p3ring1"/><div className="p3ring2"/>
        </div>
        <div className="p3tl"/><div className="p3br"/>
        ...
        {deleteConfirm && (
          <div className="p3mo">
            <div className="p3mb">
              ...
              <div className="p3mx">Delete <strong style={{color:"#e0f0ff"}}>{deleteConfirm.name}</strong> from the Dark Hour Network? This cannot be undone.</div>
              <div className="p3ma">
                <button className="p3mcb" onClick={()=>setDeleteConfirm(null)}>CANCEL</button>
                <button className="p3mdb" onClick={()=>deleteChar(deleteConfirm.id)}>DELETE</button>
              </div>
            </div>
          </div>
        )}
        ...
        {phase==="select" && (
          <div className="p3sel">
            ...
            <div className="p3grid2">
              {characters.map((c,i) => (
                <div key={c.id} className="p3card"
                  style={{"--cc":c.color,"--cg":c.color+"66",animationDelay:`${i*0.1}s`}}
                  onClick={()=>selectCharacter(c)}
                >
                  <div className="p3acts">
                    <button className="p3abtn" title="Edit" onClick={e=>openEdit(c,e)}>✎</button>
                    <button className="p3abtn del" title="Delete" onClick={e=>{e.stopPropagation();setDeleteConfirm(c);}}>✕</button>
                  </div>
                  {!c.isDefault && <div className="p3badge">CUSTOM</div>}
                  <div className="p3arc">{c.arcana} — {c.archetype}</div>
        ...
        {(phase==="create"||phase==="edit") && (
          <div className="p3cr">
            <div className="p3crh">
              <button className="p3back" onClick={()=>setPhase("select")}>← BACK</button>
            ...
            <div className="p3crb">
              ...
              <div className="p3srch">
                ...
                <div className="p3srch-row">
                  <input className="p3si" placeholder="Enter a name (e.g. Alan Turing, Nikola Tesla, Shakespeare...)" value={searchQuery}
                    onChange={e=>setSearchQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchAndBuild()}/>
              ...
              <div className="p3fg">
                ...
                <div className="p3fgroup">
                  ...
                  <select className="p3fsel" value={form.arcana} onChange={e=>{
                    ...
                    {ARCANA_OPTIONS.map(a=><option key={a.value} value={a.value}>{a.label}</option>)}
                  </select>
                </div>
                <div className="p3fgroup">
                  ...
                  <div className="p3av-row">
                    {AVATAR_OPTIONS.map(a=>(
                      <span key={a} className={`p3avopt${form.avatar===a?" active":""}`} onClick={()=>setForm(p=>({...p,avatar:a}))}>{a}</span>
                    ))}
                  </div>
                </div>
                <div className="p3fgroup full">
                  ...
                  <div className="p3cr-row">
                    {COLOR_PRESETS.map(c=>(
                      <div key={c} className={`p3cdot${form.color===c?" active":""}`}
                ...
                <div className="p3fgroup full">
                  ...
                  <input className="p3fi" placeholder="One sentence about this character" value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))}/>
                </div>
                <div className="p3fgroup full">
                  <label className="p3fl">SYSTEM PROMPT * — personality & roleplay instructions</label>
                  <textarea className="p3fta" style={{minHeight:130}} placeholder="Describe their personality, speech patterns, knowledge, mannerisms. This tells Claude how to roleplay as them." value={form.systemPrompt} onChange={e=>setForm(p=>({...p,systemPrompt:e.target.value}))}/>
                </div>
                <div className="p3fgroup full">
                  <label className="p3fl">OPENING GREETING</label>
                  <textarea className="p3fta" style={{minHeight:70}} placeholder="First line they say when the chat starts..." value={form.greeting} onChange={e=>setForm(p=>({...p,greeting:e.target.value}))}/>
                </div>
              </div>
            </div>
            <div className="p3crf">
              <button className="p3canc" onClick={()=>setPhase("select")}>CANCEL</button>
        ...
        {phase==="chat"&&char&&(
          <div className="p3chat">
            <div className="p3ch">
              <button className="p3back" onClick={()=>{setPhase("select");setSelectedChar(null);setMessages([]);}}>← BACK</button>
              ...
              <div className="p3chac">ARCANA {char.arcana}<br/><span style={{fontSize:8}}>{char.archetype}</span></div>
            </div>
            <div className="p3stat">
              <div className="p3dot"/><div className="p3stxt">DARK HOUR — CONNECTION ACTIVE</div>
            ...
            <div className="p3msgs">
              ...
              {isTyping&&(
                <div className="p3mr assistant">
                  ...
                  <div className="p3tyb"><TypingIndicator color={char.color}/></div>
                </div>
              )}
            ...
            <div className="p3inp">
              ...
              <div className="p3inpw">
                <textarea ref={inputRef} className="p3ta" placeholder={`Speak to ${char.name}...`}
                  value={input} onChange={e=>setInput(e.target.value)}
                  onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMessage();}}} rows={1}/>
                <button className="p3send" onClick={sendMessage} disabled={isTyping||!input.trim()}>SEND</button>
              </div>
            </div>
          </div>
        )}
